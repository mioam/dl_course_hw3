...
class Shuffle(nn.Module):
    ...
    def forward(self, inputs, mode='direct'):
        if mode == 'direct':
            ...
            #############################################
            return inputs[:,self.perm], torch.zeros((inputs.shape[0],1), device=inputs.device)
        else:
            #############################################
            # TODO complete code here; z - > x
            # return x = f^-1(z) and logdet, x has the same shape with z, logdet has the shape (batch size, 1)
            #############################################
            return inputs[:,self.inv_perm], torch.zeros((inputs.shape[0],1), device=inputs.device)
...
class FlowSequential(nn.Sequential):
    ...
    def log_probs(self, inputs, pre_process=True):
        ...
        log_prob = -u**2 / 2 - math.log(2*math.pi) / 2
